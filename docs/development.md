...
For production builds, set `SENTRY_ORG`, `SENTRY_PROJECT`, and `SENTRY_AUTH_TOKEN` (auth token needs `project:releases` scope). `withSentryConfig` in `next.config.ts` uploads source maps at build time so server-side stack traces deminify, then deletes the `.map` files so they don't ship to the browser. Session Replay is configured to capture only sessions that hit an error (`replaysOnErrorSampleRate: 1.0`, session sample rate `0`); the replay integration masks all text + inputs and blocks media so account numbers and balances never leave the browser.
